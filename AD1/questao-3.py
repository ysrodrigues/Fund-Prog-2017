...
def createMatrix3(m, l, c):
	matrix = [
		...
		[m[l+1]el[c+1], m[l+1][c], m[l+1][c+1]]
	]
	return matrix

# Função que verifica se tem alguma matrix 3x3 dentro da matrix dada, onde o valor central é menor que todos
def getMatrixMiddle(matrix):  ao seu redor
	if len(matrix) >= 3: # Se a matrix for menor que a 3x3 não faz nd
		maxline = len(matrix)-1
		maxcolumn = len(matrix[0])-1
		line = 1
		while line < len(matrix)-1: # Varre a matrix da linha 1 ate o l-1 linhas, onde l = total de linhas
			column = 1
			while column < maxcolumn: # Varre a matrix da coluna 1 ate a c-1 colunas, onde c = total de colunas
				value = matrix[line][column]
				if value < matrix[line][column+1]: # Verifica se o valor a frente é menor
					if value < matrix[line-1][column] and value < matrix[line+1][column]: # Verifica os valores na horizontal
						if value < matrix[line-1][column-1] and value < matrix[line+1][column-1]: # Verifica os valores na diagonal
							if value < matrix[line-1][column+1] and value < matrix[line+1][column+1]: # Verifica os valores na diagonal
								if column == 1: # Verifica se o valor a atras é menor, caso seja coluna 1
									if value < matrix[line][column-1]:
										matrix_3x3 = createMatrix3(matrix, line, column)
...
def printMatrix(matrix): # Função que imprime na tela de forma convencional uma matrix
	for l in matrix:
		for c in l:
			print (c, end=" ")
		print()
